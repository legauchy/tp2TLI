...
public class Grapher extends JPanel {
    ...
    public Grapher() {
        ...
        ymin = -1.5;
        ymax = 1.5;
        GrapherMouseListener mouseListener = new GrapherMouseListener();
    ...
    private class GrapherMouseListener implements MouseMotionListener, MouseListener, MouseWheelListener {
        ...
        public void mousePressed(MouseEvent e) {
            switch (state) {
                case IDLE:
                    ...
                    posY = e.getY();
                    if (e.getButton() == 1) {
                        state = STATES.PRESS_OR_DRAG_LEFT;
                    } else if (e.getButton() == 3) {
                        state = STATES.PRESS_OR_DRAG_RIGHT;
                    }
                    break;
                default:
                    break;
        ...
        public void mouseDragged(MouseEvent e) {
            switch (state) {
                case PRESS_OR_DRAG_LEFT:
                    state = STATES.DRAG_LEFT;
                    setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
                case DRAG_LEFT:
                    int decalX = e.getX() - posX;
                    int decalY = e.getY() - posY;
                    translate(decalX, decalY);
                    posX = e.getX();
                    posY = e.getY();
                    break;

                case PRESS_OR_DRAG_RIGHT:
                    state = STATES.DRAG_RIGHT;
                case DRAG_RIGHT:
                    Point dragPoint = e.getPoint();
                    int x = Math.min(posX, dragPoint.x);
                    int y = Math.min(posY, dragPoint.y);
                    int width = Math.max(posX - dragPoint.x, dragPoint.x - posX);
                    int height = Math.max(posY - dragPoint.y, dragPoint.y - posY);
        ...
        public void mouseReleased(MouseEvent e) {
            switch (state) {
                ...
                case DRAG_RIGHT:
                    zoom(selectionRectangle.getLocation(),
                            new Point(selectionRectangle.x + selectionRectangle.width,
